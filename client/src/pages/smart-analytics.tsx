...
export default function SmartAnalytics() {
  ...
  // Query untuk data monthly trends dari API
  const { data: monthlyTrendsData, isLoading: isLoadingTrends } = useQuery({
    queryKey: ['monthly-trends'],
    queryFn: async () => {
      const response = await fetch('https://employee-insights-api.adityalasika.workers.dev/api/employee-insights/monthly-trends');
      if (!response.ok) {
  ...
  // Query untuk data statistik dari employee_insights table
  const { data: employeeStats, isLoading: isLoadingStats } = useQuery({
    queryKey: ['employee-insights-stats'],
    queryFn: async () => {
      const response = await fetch('https://employee-insights-api.adityalasika.workers.dev/api/employee-insights/stats');
      if (!response.ok) {
        throw new Error('Failed to fetch employee insights statistics');
      }
      const result = await response.json();
      return result;
    },
  ...
  // Query untuk mendapatkan daftar witel unik
  const { data: witelOptions } = useQuery({
    queryKey: ['witel-options'],
    queryFn: async () => {
      const response = await fetch('https://employee-insights-api.adityalasika.workers.dev/api/employee-insights/paginated?limit=1000');
      if (!response.ok) throw new Error('Failed to fetch witel options');
      const result = await response.json();
      const witels = [...new Set(result.data.map((item: any) => item.witel).filter(Boolean))].sort();
      return witels;
    },
    staleTime: 10 * 60 * 1000, // Cache for 10 minutes
  });

  // Query untuk mendapatkan daftar source unik
  const { data: sourceOptions } = useQuery({
    queryKey: ['source-options'],
    queryFn: async () => {
      const response = await fetch('https://employee-insights-api.adityalasika.workers.dev/api/employee-insights/paginated?limit=1000');
      if (!response.ok) throw new Error('Failed to fetch source options');
      const result = await response.json();
      const sources = [...new Set(result.data.map((item: any) => item.sourceData).filter(Boolean))].sort();
      return sources;
    },
    staleTime: 10 * 60 * 1000, // Cache for 10 minutes
  });
