...
const AIConclusion: React.FC<AIConclusionProps> = ({
  ...
  const parseAIContent = (text: string): ParsedAIContent => {
    ...
    const summaryMatch = text.match(/\*\*SUMMARY EKSEKUTIF\*\*:?\s*(.*?)(?=\*\*ANALISIS MENDALAM\*\*|$)/is);
    const analysisMatch = text.match(/\*\*ANALISIS MENDALAM\*\*:?\s*(.*?)(?=\*\*UPAYA PENGEMBANGAN\*\*|$)/is);
    const improvementMatch = text.match(/\*\*UPAYA PENGEMBANGAN\*\*:?\s*(.*?)$/is);
