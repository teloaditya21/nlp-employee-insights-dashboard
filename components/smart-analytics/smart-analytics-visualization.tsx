...
import PredictiveInsights from "@/components/charts/predictive-insights";
import AdvancedMetrics from "@/components/charts/advanced-metrics";

interface SmartAnalyticsVisualizationProps {
  insights: any[];
  searchTerm: string;
...
export default function SmartAnalyticsVisualization({
  insights,
  searchTerm,
  onSearchChange,
  onClearSearch,
  ...
  setPresetRange,
  clearDateRange,
}: SmartAnalyticsVisualizationProps) {
  
  // Process insights data for advanced analytics
  const analyticsData = useMemo(() => {
    ...
    // Prepare data for different analytics visualizations
    const sentimentAnalysis = filteredInsights.map(insight => ({
      topic: insight.word_insight,
      ...
      confidence: Math.max(insight.positif_percentage, insight.negatif_percentage, insight.netral_percentage),
    }));

    const trendData = filteredInsights
      .slice(0, 20)
      .map((insight, index) => ({
    ...
    const correlationData = filteredInsights
      .slice(0, 10)
      .map(insight => ({
        topic: insight.word_insight,
        engagement: insight.total_count,
        sentiment_score: insight.positif_percentage - insight.negatif_percentage,
        volume: insight.total_count,
        impact: (insight.positif_percentage + insight.negatif_percentage) * insight.total_count / 100,
      }));

    const predictiveData = filteredInsights
      .slice(0, 15)
      .map((insight, index) => ({
        topic: insight.word_insight,
        current: insight.total_count,
        predicted: insight.total_count * (1 + (Math.random() - 0.5) * 0.3), // Simulated prediction
        confidence: 0.7 + Math.random() * 0.25, // Simulated confidence
        trend: (Math.random() > 0.5 ? 'up' : 'down') as 'up' | 'down',
      }));

    const advancedMetrics = {
      sentimentStability: filteredInsights.reduce((acc, insight) => {
        const variance = Math.pow(insight.positif_percentage - 33.33, 2) + 
                        Math.pow(insight.negatif_percentage - 33.33, 2) + 
                        Math.pow(insight.netral_percentage - 33.33, 2);
        return acc + variance;
      }, 0) / filteredInsights.length,
      
      topicDiversity: uniqueTopics.length,
      
      engagementRate: filteredInsights.reduce((acc, insight) => acc + insight.total_count, 0) / filteredInsights.length,
      
      polarizationIndex: filteredInsights.reduce((acc, insight) => {
        const polarization = Math.abs(insight.positif_percentage - insight.negatif_percentage);
        return acc + polarization;
      }, 0) / filteredInsights.length,
    };
  ...
  }, [insights, selectedTopics, uniqueTopics]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[12px] p-4 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
          ...
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder="Cari insights untuk analisis... (contoh: wellness, gaji, fasilitas)"
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-[12px] p-6 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
          <h3 className="text-lg font-semibold mb-4">Advanced Sentiment Analysis</h3>
          <SentimentAnalysisChart data={analyticsData.sentimentAnalysis} />
        </div>
        ...
        <div className="bg-white rounded-[12px] p-6 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
          ...
          <CorrelationMatrix data={analyticsData.correlationData} />
        </div>

        {/* Predictive Insights */}
        <div className="bg-white rounded-[12px] p-6 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
          <h3 className="text-lg font-semibold mb-4">Predictive Insights</h3>
          <PredictiveInsights data={analyticsData.predictiveData} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-[12px] p-6 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
        ...
        <AdvancedMetrics metrics={analyticsData.advancedMetrics} />
      </div>
      ...
      <div className="bg-white rounded-[12px] p-6 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{analyticsData.totalInsights}</div>
            <div className="text-sm text-gray-600">Analyzed Insights</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {analyticsData.advancedMetrics.sentimentStability.toFixed(1)}
            </div>
            <div className="text-sm text-gray-600">Sentiment Stability</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {analyticsData.advancedMetrics.topicDiversity}
            </div>
            <div className="text-sm text-gray-600">Topic Diversity</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">
              {analyticsData.advancedMetrics.polarizationIndex.toFixed(1)}%
            </div>
            <div className="text-sm text-gray-600">Polarization Index</div>
          </div>
        </div>
      </div>
    </div>
